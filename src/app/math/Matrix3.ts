...
class Matrix3 {
  elements: number[];// always 9 elements
  ...
  //
  scaleSingle( s:number ) {

    this.premultiply( _m3.makeScale( s, s ) );
  ...
  toSvgTransform(){
    ...
    const a21 = ae[ 1 ], a22 = ae[ 4 ], a23 = ae[ 7 ];
    const a31 = ae[ 2 ], a32 = ae[ 5 ], a33 = ae[ 8 ];
    // todo how to handle that the lower once have to be 0 and the lower right be 1??
    return `matrix(${a11} ${a21} ${a12} ${a22} ${a13} ${a23})`
  }
